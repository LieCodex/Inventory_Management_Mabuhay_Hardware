        ...
        <div class="mx-auto w-full max-w-7xl space-y-6 p-4 md:p-6">
            ...
            <div class="grid gap-6 lg:grid-cols-3">
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <section class="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
                        <h3 class="mb-4 font-semibold text-zinc-800 dark:text-zinc-100">Pending Password Resets</h3>
                        <div class="overflow-x-auto">
                            <table class="w-full text-left text-sm">
                                <thead class="text-zinc-500 uppercase text-xs border-b border-zinc-100 dark:border-zinc-800">
                                    <tr>
                                        ...
                                        <th class="py-3 px-2">Requested</th>
                                        <th class="py-3 px-2 text-right">Actions</th>
                                    </tr>
                                </thead>
                                <tbody class="text-zinc-700 dark:text-zinc-200">
                                    @forelse($pendingResets as $request)
                                    <tr class="border-t border-zinc-100 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                                        <td class="py-3 px-2 font-medium">{{ $request->user->name }}</td>
                                        <td class="py-3 px-2 text-zinc-500">{{ $request->user->email }}</td>
                                        <td class="py-3 px-2 text-zinc-500">{{ $request->created_at->diffForHumans() }}</td>
                                        <td class="py-3 px-2 text-right flex justify-end gap-3">
                                            <form action="{{ route('admin.resets.approve', $request) }}" method="POST">
                                                @csrf
                                                <button type="submit" class="text-emerald-600 hover:text-emerald-700 font-medium">Approve</button>
                                            </form>
                                            <span class="text-zinc-300 dark:text-zinc-600">|</span>
                                            <form action="{{ route('admin.resets.reject', $request) }}" method="POST">
                                                @csrf
                                                <button type="submit" class="text-rose-500 hover:text-rose-700 font-medium">Reject</button>
                                            </form>
                                        </td>
                                    </tr>
                                    @empty
                                    <tr>
                                        <td colspan="4" class="py-4 px-2 text-center text-zinc-500">No pending password reset requests.</td>
                                    </tr>
                                    @endforelse
                                </tbody>
                            </table>
