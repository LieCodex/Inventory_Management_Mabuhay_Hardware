    ...
    <div class="space-y-6" x-data="{ activeTab: new URLSearchParams(location.search).has('page') ? 'deliveries' : 'overview' }">
        ...
        <div class="rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex items-center gap-3">
                    @livewire(\App\Livewire\SupplierEdit::class, ['supplierId' => $supplier->id])
                    @if(!empty($supplier->email))
                        <a href="mailto:{{ $supplier->email }}?subject={{ urlencode('Supplier Inquiry - ' . $supplier->company_name) }}"
                           class="rounded-lg border border-zinc-200 bg-zinc-50 px-4 py-2 text-sm font-medium text-zinc-700 transition-colors hover:bg-zinc-100 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-200 dark:hover:bg-zinc-700">
                            Contact Supplier
                        </a>
                    @elseif(!empty($supplier->contact_number))
                        <a href="tel:{{ preg_replace('/\\s+/', '', $supplier->contact_number) }}"
                           class="rounded-lg border border-zinc-200 bg-zinc-50 px-4 py-2 text-sm font-medium text-zinc-700 transition-colors hover:bg-zinc-100 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-200 dark:hover:bg-zinc-700">
                            Call Supplier
                        </a>
                    @else
                        <button disabled class="cursor-not-allowed opacity-50 rounded-lg border border-zinc-200 bg-zinc-50 px-4 py-2 text-sm font-medium text-zinc-700 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-200">
                            No contact info
                        </button>
                    @endif
                </div>
